...
app.use(cors());
// Middleware method-override untuk menafsirkan _method dari body sebagai HTTP method
...
const authenticateToken = (req, res, next) => {
    ...
    jwt.verify(token, 'SECRET_KEY_YANG_SANGAT_RAHASIA', (err, user) => {
        if (err) {
...
const uploadAndValidate = (fieldName) => {
    return (req, res, next) => {
        upload.single(fieldName)(req, res, (err) => {
            ...
            // Jika file tidak ada, lewati validasi file jika diperbolehkan
...
// Rute Ambil Biodata Pengguna (SEKARANG AKAN MENGAMBIL DATA BIODATA DENGAN BENAR)
app.get('/api/user', authenticateToken, async (req, res) => {
    try {
        ...
        // Jika user ditemukan, kirim datanya
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (sub_district !== undefined) {
                updates.sub_district = sub_district;
            }
            if (regency !== undefined) {
                updates.regency = regency;
            }
            ...
            if (identity_card_path) {
                updates.identity_card_path = identity_card_path;
            }
...
app.post(
    ...
    authenticateToken,
    uploadAndValidate('cv'), // Gunakan middleware baru
    [
    ...
    async (req, res) => {
        ...
        const cvPath = req.file.path; // Pasti ada karena middleware uploadAndValidate
